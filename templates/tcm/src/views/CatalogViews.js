...
export function renderAllServicesView() {
    ...
    if (filtered.length === 0) {
        gridHtml = `
            <div class="col-span-full text-center py-16 bg-white rounded-3xl border border-[#e7dfd1] p-8 shadow-sm">
                <span class="material-symbols-outlined text-4xl text-[#164e3f]/40 mb-3">search_off</span>
        ...
        filtered.forEach(srv => {
            ...
            const discountBadgeHtml = discountPercent > 0 
                ...
                : `<div class="absolute top-4 left-4 bg-white/95 backdrop-blur-sm text-[#164e3f] px-3 py-1 rounded-full font-bold text-[9px] uppercase tracking-wider shadow-sm border border-[#164e3f]/20">${badgeLabel}</div>`;
            ...
            if (isPackage && srv.services && srv.services.length > 0) {
                ...
                includesHtml = `
                    <div class="mt-3 mb-5 p-3.5 bg-[#faf8f5] rounded-2xl border border-[#e7dfd1]">
                        <div class="text-[10px] font-bold uppercase tracking-wider text-[#164e3f] mb-2 flex items-center gap-1 font-semibold">
                            <span class="material-symbols-outlined text-[14px] text-[#164e3f]">featured_play_list</span>
                        ...
                        <ul class="text-[10.5px] text-slate-700 space-y-1.5 font-medium">
                            ${subNames.map(name => `<li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-[#164e3f]/70 shrink-0"></span><span class="truncate">${name}</span>${sessStr}</li>`).join('')}
                        </ul>
                    </div>
                `;
            }
            ...
            gridHtml += `
                <div class="tcm-card overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col justify-between relative bg-white">
                    ${srv.bestValue ? `<div class="absolute top-0 right-0 bg-[#c59b27] text-white px-4 py-1 rounded-bl-lg font-label-caps text-[9px] font-bold uppercase tracking-wider z-10">${state.language === 'ms' ? 'NILAI TERBAIK' : (state.language === 'zh' ? '超值优选' : 'BEST VALUE')}</div>` : ''}
                    ...
                    <div class="p-6 flex flex-col justify-between flex-grow">
                        <div>
                            <h3 class="font-serif text-base font-bold text-[#0f3d32] mb-2 line-clamp-1 leading-snug">${getServiceTranslation(srv.id, 'name', srv.name)}</h3>
                        ...
                        <div class="flex justify-between items-center border-t border-slate-100 pt-4 mt-auto">
                            <div class="flex flex-col justify-end">
                                ...
                                <span class="font-serif font-bold text-[#0f3d32] text-base">${currency} ${srv.price}</span>
                            </div>
    ...
    container.innerHTML = `
        <div class="space-y-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-[#e7dfd1]">
                <div class="flex items-center gap-3">
                    <button onclick="navigateTo('services-catalog')" class="w-10 h-10 rounded-full bg-[#164e3f]/10 hover:bg-[#164e3f]/20 text-[#164e3f] flex items-center justify-center transition-colors">
                    ...
                    <div>
                        ...
                        <h1 class="font-serif text-2xl md:text-3xl text-[#0f3d32] font-bold">
                ...
                <div class="relative w-full md:w-80">
                    ...
                    <input type="text" id="all-services-search" placeholder="${state.language === 'ms' ? 'Cari rawatan TCM...' : (state.language === 'zh' ? '搜索诊疗项目...' : 'Search TCM therapies...')}" value="${state.searchQuery}" oninput="searchAllServices(this.value)" class="w-full pl-10 pr-4 py-2.5 rounded-full border border-[#e7dfd1] focus:outline-none focus:border-[#164e3f] text-xs font-semibold text-slate-800 bg-white">
                    ${state.searchQuery ? `
