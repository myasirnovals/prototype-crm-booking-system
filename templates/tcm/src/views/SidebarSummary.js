...
export function renderSidebarSummary() {
    ...
    let html = `
        ...
        <div class="flex flex-col gap-5">
            ...
            <div class="flex gap-3 items-start">
                <div class="w-10 h-10 rounded-xl bg-[#046bd2]/10 flex items-center justify-center shrink-0 text-[#046bd2]">
            ...
            <!-- Therapist Info -->
            <div class="flex gap-3 items-start">
                <div class="w-10 h-10 rounded-xl bg-[#f0f5fa] flex items-center justify-center shrink-0 overflow-hidden border border-[#e2e8f0]">
                    ${therapist && therapist.image ? `
                        <img class="w-full h-full object-cover" src="${therapist.image}">
                    ` : `
                        <div class="w-10 h-10 rounded-xl bg-[#046bd2]/10 flex items-center justify-center text-[#046bd2] shrink-0">
        ...
        <div class="mt-6 pt-6 border-t border-[#e2e8f0]">
            ...
            ${state.currentView === 'confirm-booking' ? `
                <div class="mt-4 p-3 bg-[#f0f5fa] rounded-2xl border border-[#e2e8f0] text-left space-y-2">
                    ...
                    <div class="text-[10px] text-slate-600 leading-tight pt-1.5 border-t border-[#e2e8f0] flex items-start gap-1">
                        <span class="material-symbols-outlined text-[13px] shrink-0 text-[#ff6900]">info</span>
    ...
    if (state.currentView === 'select-service') {
        html += `
            <div class="hidden md:flex justify-between items-center mt-6 pt-6 border-t border-[#e2e8f0] gap-3">
                <button onclick="resetBookingFlow()" class="px-5 py-2 rounded-full border border-[#046bd2]/40 text-[#046bd2] hover:bg-[#046bd2]/5 text-xs font-bold flex items-center gap-1.5 transition-all w-1/2 justify-center">
        ...
        html += `
            <div class="hidden md:flex justify-between items-center mt-6 pt-6 border-t border-[#e2e8f0] gap-3">
                <button onclick="navigateTo('select-service')" class="px-5 py-2 rounded-full border border-[#046bd2]/40 text-[#046bd2] hover:bg-[#046bd2]/5 text-xs font-bold flex items-center gap-1.5 transition-all w-1/2 justify-center">
                    <span class="material-symbols-outlined text-xs">arrow_back</span> ${t('btn_back')}
                </button>
        ...
        html += `
            <div class="hidden md:flex justify-between items-center mt-6 pt-6 border-t border-[#e2e8f0] gap-3">
                <button onclick="navigateTo('select-therapist')" class="px-5 py-2 rounded-full border border-[#046bd2]/40 text-[#046bd2] hover:bg-[#046bd2]/5 text-xs font-bold flex items-center gap-1.5 transition-all w-1/2 justify-center">
                    <span class="material-symbols-outlined text-xs">arrow_back</span> ${t('btn_back')}
                </button>
        ...
        html += `
            <div class="flex flex-col gap-3 mt-6 pt-6 border-t border-[#e2e8f0]">
                ...
                <button onclick="prevStep(4)" class="w-full py-2.5 rounded-full border border-[#046bd2]/40 text-[#046bd2] hover:bg-[#046bd2]/5 text-xs font-bold transition-all text-center">
                    ${t('btn_back')}
                </button>
            </div>
        `;
    ...
    container.innerHTML = `
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8 text-left">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            ${therapist && therapist.image ? `
                                <img class="w-7 h-7 rounded-xl object-cover border border-[#e2e8f0]" src="${therapist.image}">
                            ` : `
                                <div class="w-7 h-7 rounded-xl bg-[#046bd2]/10 flex items-center justify-center">
            ...
            <div class="md:col-span-5 flex flex-col justify-center">
                <div class="flex flex-col items-center gap-4 bg-[#f0f5fa] p-6 rounded-2xl border border-[#e2e8f0] w-full max-w-[240px] mx-auto md:ml-auto">
                    <div class="w-32 h-32 bg-[#045cb4] rounded-xl p-2 flex items-center justify-center shrink-0 shadow-md">
                    ...
                    <button class="w-full py-2 bg-white border border-[#e2e8f0] text-[#045cb4] hover:bg-[#f0f5fa] font-bold text-[11px] rounded-full flex items-center justify-center gap-1.5 transition-all shadow-sm">
